...
def main():
    # 用户输入
    while True:
        choice = input("选择解密还是加密: (e)ncrypt, (d)ecrypt, (q)uit: ").lower()
        if choice == 'q':
            break
        elif choice in ['e', 'd']:
            # 获取密钥
            key = int(input("输入密钥 (as a 4-digit hex number): "), 16)
            # 获取明文或密文
            if choice == 'e':
                plaintext = int(input("输入加密文件： (as a 4-digit hex number): "), 16)
                aes = SimplifiedAES(key)
                ciphertext = aes.encrypt(plaintext)
                print(f"Ciphertext: {ciphertext:04X}")
            elif choice == 'd':
                ciphertext = int(input("输入解密文件： (as a 4-digit hex number): "), 16)
                aes = SimplifiedAES(key)
